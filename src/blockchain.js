        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (currentTime - messageTime < 300) {
                    ...
                    reject('Time elapsed must be less than 5 minutes');
                }
            } catch (error) {
                console.log(error);
                reject(error);
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async (block) => {
                ...
                if (!isValid) {
                    errorLog.push(`Block ${block.hash} at height ${block.height} is not valid`);
                };
                if (block.height > 0) {
                    if (block.previousBlockHash !== self.chain[block.height-1].hash) {
                        errorLog.push(`Previous hash in block ${block.hash} at height ${block.height} is not valid`);
                    }
                }
            });
            resolve(errorLog);
